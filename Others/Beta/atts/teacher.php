<?php
	...
	if (!isset($_SESSION) || !isset($_SESSION['id_number']) || $_SESSION['type'] != "teacher"){
		header("location: index.php");
	}

	$sql = "select * from system where action = '".$_SESSION['id_number']."'";
	$result = $conn->query($sql);
	if($row = $result->fetch_assoc()){
		$deactivate = $row['value'];
		if ($deactivate == 'on') {
			$deactivate = 'attendance2';
		}else{
...
<body>
	...
	<div class="sidemenu">
		...
		<ul>
			...
			<?php echo "<li id='".$deactivate."' class='btn btn-block'>Attendance</li>"; ?>
			<li id="setting" class="btn btn-block">Setting</li>
			<li id="logout" class="btn btn-block">Logout</li>
		</ul>
	</div>
	<div class="content">
		<iframe src="teacher/dashboard.php" name="mframe"></iframe>
	</div>
