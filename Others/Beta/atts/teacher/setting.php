<?php 
	...
	if (isset($_GET['msg'])) {
		if ($_GET['msg'] != 'on') {
			echo "<div class='btn-danger pl-4 pt-2 pb-2 lg'>Error: <br><div class='pl-4 small'>* Face Recognition is Deactivated<br>* Data is uptodate.</div></div>";
		}else{
			echo "<div class='btn-success pl-4 pt-2 pb-2 lg'>Notice: <br><div class='pl-4 small'>* Face Recognition is Activated.<br>* Data is uptodate.</div></div>";
		}
		echo "<script>window.open('../teacher.php', '_parent')</script>";
	...
	$sql = "select * from system where action = '$id_number'";
	$result = $conn->query($sql);
	if($row = $result->fetch_assoc()){
	...
	if (isset($_POST['update'])) {
		...
		if($conn->query($sql)===TRUE){
			$sql = "select * from system where action = '$id_number' limit 1";
			$result = $conn->query($sql);
			if($row = $result->fetch_assoc()){
				$sql = "update system set value='$deact' where action = '$id_number' ";
				if($conn->query($sql)===TRUE){
					echo "";
				}
			}else{
				$sql = "insert into system (action, value) values ('$id_number', '$deact')";
				if($conn->query($sql)===TRUE){
					echo "";
				}
			}
		...
		<div class="row">
			<div class="col-lg-6 mt-5">
				...
				<form class="form-group mt-5" action="<?php $_SERVER['PHP_SELF'] ?>" method="POST">
					...
					<label class="ml-2" for="deact">Face Recognition Attendance</label>
